...
def lgb_best(X,y):
    ...
    if max(y)>1 :
        num_class = max(y)+1
    param ={'num_class':[num_class],'n_estimators':range(10,71,10),'feature_fraction': [0.6,0.7,0.8,0.9,1.0],
            'bagging_fraction': [0.6,0.7,0.8,0.9,1.0],
        'min_data_in_leaf':[1,int(0.005*num_sample+1),int(0.01*num_sample+1),int(0.02*num_sample+1),int(0.05*num_sample+1)],
            'max_depth':[depth,int(math.sqrt(depth)),int(math.log2(depth))]}
    gsearch= GridSearchCV(estimator =lgb.LGBMClassifier(random_state=10), 
                       param_grid =param,scoring='accuracy',cv=5, n_jobs = 5)
    gsearch.fit(X,y)
    best = gsearch.best_params_
    lgb_best = lgb.LGBMClassifier(random_state=10,n_estimators = best["n_estimators"],bagging_fraction = best["bagging_fraction"],feature_fraction = best["feature_fraction"],
                                    min_data_in_leaf = best["min_data_in_leaf"],max_depth = best["max_depth"])
    lgb_best.num_class = num_class
    return(lgb_best)
...
def LGB_err_bias_var_clc_def(X,y,n = 100,train_size_p = 0.6,pool_size_p = 0.15,test_size_p = 0.15):
    pool_X, test_X, pool_y, test_y = train_test_split(X, y,train_size = train_size_p ,test_size = test_size_p)
    num_class = 1
    if max(y)>1 :
        num_class = max(y)+1
    clc = lgb.LGBMClassifier(n_jobs = 6,random_state = 10)
    clc.num_class = num_class
    clc_best =  lgb_best(pool_X,pool_y)
    clc_best.num_class = num_class
    clc_best.n_jobs = 6
...
def bvr_cal(train,pool,test):
    ...
    for classification_dataset in classification_dataset_names:
        ...
        stat = LGB_err_bias_var_clc_def(X,y,100,train,pool,test)
